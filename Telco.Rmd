...
# First-Level Models
## Logistic Regression - NEED TO UPDATE INTERPRETATION WITH IN-LINE CODE
### Simple LR
```{r}
set.seed(300)
library(caret)
...
## ANN - ELABORATE ON THE INTERPRATION
```{r, cache = TRUE}
set.seed(300)
...
## Decision Tree - UPDATE INTERPRETATIONS
### Simple DT
```{r, cache = TRUE}
set.seed(300)
...
## Support Vector Machine (SVM) - UPDATE INTERPRETATIONS
### SVM Linear
```{r, cache = TRUE}
set.seed(300)
...
## First-Layer Results - HUGE INTERPRETATION NEEDED HERE - SUMMARY OF ALL RESULTS AND EXPLANATION FOR WHICH MODELS WE CHOSE FOR THE SECOND LAYER
...
```
<br>
The average trailing twelve months' net profit margin for companies in the telecommunications sector, as of Q3 2022, is approximately 12.3%. Gross profit margins for the sector can run as high as 70% to 80%, but extremely high overhead expenses erode much of that initial profit balance ("Communications Services Industry Profitability by Quarter, Gross, Operating and Net Margin From 3 Q 2022"). Assuming a 75% gross profit margin, TelCo's monthly gross profit per user makes up $`r round(telco_monthly_gppu,2)`. 
