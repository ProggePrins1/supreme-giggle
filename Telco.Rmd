...
binary_lm <- ifelse(predict(predictionmodelt2, newdata = telco_test, type = "response") < 0.5, 0, 1)
...
For this section, we will check if we can predict with 90% accuracy whether someone will churn out of our telco offering. To do so, we will first build an ANN, kNN, LR, and decision tree prediction model using cross-validation, and then use those outputs to create an ensemble model. 
...
binary_ann <- as.numeric(ifelse(predicted_churn < 0.5, 0, 1))

# CrossTable and Confusion Matrix

CrossTable(telco_test_normal$Churn, binary_ann, prop.chisq = FALSE)

confusionMatrix(data = as.factor(binary_ann), reference = as.factor(telco_test_normal$Churn), positive = "1")
```

### kNN
```{r}
# Set k to sqrt of train set.

k <- round(sqrt(nrow(telco_train_normal)), 0)
# k = 73 with a 75/25 split.

#Exclude churn from train and test, labels is just the churn column, k = k (73 for now)
telco_pred_knn <- knn(train = telco_train_normal[-31], test = telco_test_normal[-31], cl = telco_train_normal$Churn, k)


# Create crosstable, change headers of the table
CrossTable(as.factor(telco_test_normal$Churn), telco_pred_knn, prop.chisq = F, dnn = c("Real Churn", "Predicted Churn"))

# Create a confusion matrix
confusionMatrix(data = telco_pred_knn, reference = as.factor(telco_test_normal$Churn), positive = "1")
```
...
confusionMatrix(data = dtpred, reference = as.factor(telco_test_normal$Churn), positive = "1")
```
...
svm_predict <- predict(svm_model, telco_test_normal)

binary_svm <- ifelse(svm_predict < 0.35, 0, 1)

CrossTable(telco_test_normal$Churn, binary_svm, prop.chisq = F, dnn = c('Actual Churn', 'Predicted Churn'))
...
```{r}
# Let's combine everything into one big dataframe for our stacked model.

# Let's change telco_pred_knn into a more intuitive numeric
telco_pred_knn <- ifelse(telco_pred_knn == 0, 0, 1)
combineddf <- data.frame(binary_lm, binary_ann, telco_pred_knn, dtpred, binary_svm, telco_test_normal$Churn)
...
confusionMatrix(data = as.factor(sm_telco_test$Actual), reference = as.factor(dt_pred_comb), positive  = "1")
```
...
In the first round of our prediction models
```{r}

```

# Can we Estimate Customer Value More Accurately by using Monthly Bill Rates?
## Too many levels? Collapse everything after the first three levels with the most observations