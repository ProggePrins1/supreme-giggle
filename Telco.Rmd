...
## Let's remove the insignificant variables for now


telco_train$gender <- NULL
telco_train$SeniorCitizen <- NULL
telco_train$Partner <- NULL
telco_train$Dependents <- NULL
telco_train$PhoneService <- NULL
telco_train$MultipleLines <- NULL
telco_train$OnlineSecurity <- NULL
telco_train$OnlineBackup <- NULL
telco_train$DeviceProtection <- NULL
telco_train$TechSupport <- NULL
telco_train$StreamingTV <- NULL
telco_train$StreamingMovies <- NULL
telco_train$MonthlyCharges <- NULL

## Let's run the model again now that we have excluded all the nonsignificant variabless, with a threshhold of 5%

predictionmodelt2 <- glm(Churn ~ ., data = telco_train, family = "binomial")
...
What immediately stands out in this model is the high number of false positives (120) where our model predicts that a customer would be retained, when it isn't the case in reality. Since this is an initial and relatively simple model, that is okay for now, as we will continue to work with this dataset to improve the prediction accuracy and bring down the false positives at the same time.