...
telco_modelm <- as.data.frame(model.matrix(~., -20, data = telco))
...
telco_test   <- telco[(0.75*nrow(telco)):nrow(telco),]

#telco_sample_normal <- sample(c(TRUE, FALSE), nrow(telco_normal), replace=TRUE, prob=c(0.75,0.25))

telco_train_normal <- telco_normal[1:(0.75*nrow(telco)), ]
telco_test_normal <- telco_normal[(0.75*nrow(telco)):nrow(telco),]
...
```{r}
# Let's set a seed so that we have consistent results
set.seed(357)
...
### Characteristics of Each Cluster & Churn Rates per Cluster
```{r}

```

> Text describing each cluster
...
telco_ann <- neuralnet(Churn ~., data = telco_train)
